...
export function ShareButtons({ repo }: ShareButtonsProps) {
  ...
  const stars = repo.stargazers_count ? `⭐ ${formatNumber(repo.stargazers_count)} stars` : '';
  const language = repo.language ? `• ${repo.language}` : '';
  const shareText = `Check out ${shareData.title}: ${shareData.description} ${stars} ${language}`.trim();
  ...
  const shareOnTwitter = () => {
    const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(shareData.url)}`;
